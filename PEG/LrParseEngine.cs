...
namespace PEG
{
    ...
    public class LrParseEngine : ParseEngine
    {
        ...
        private IEnumerable<OutputRecord> LeftRecursion = new List<OutputRecord>();
        ...
        public override bool IsFailure(IEnumerable<OutputRecord> output)
        {
            return output == null || output == LeftRecursion;
        }

        public ParseException CreateException()
        {
            Terminal terminal = Input[MaxPosition];
            int realPosition = MaxPosition;
            if (terminal is Token)
                realPosition = ((Token)terminal).Position;
